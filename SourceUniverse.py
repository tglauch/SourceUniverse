...
def calc_dN(LF,logL0,z0, logLbinwidth, zbinwidth, N_tot, int_norm, cosmology):
    LF_val=LF(z0,logL0)
    return (4*np.pi*LF_val*diff_comoving_volume(z0, **cosmology)/int_norm)*logLbinwidth*zbinwidth*N_tot

# Physics Settings

def calc_pdf(density=1e-7, L_nu=1e50, sigma=1, gamma=2.19, logMu_range = [-10,6], N_Mu_bins = 200, z_limits = [0.04, 10.], nzbins = 120, Lum_limits = [1e45,1e54], nLbins = 120, flux_to_mu=10763342917.859608):
    """
    Parameter:
        - density in 1/Mpc^3
        - L_nu in erg/yr
    ...
    LF = lambda z, logL : redshift_evolution(z)*np.log(10)*10**logL*lognorm.pdf(10**logL, np.log(10)*sigma, scale = L_nu*np.exp(-0.5*(np.log(10)*sigma)**2))
    ...
    for z_count, z in enumerate(zs):
        bz=calc_conversion_factor(z, gamma) # Conversion Factor for given z
        ...
        for l_count, lum in enumerate(Ls):
                ...
                dN=calc_dN(LF, lum, z, deltaL, deltaz, N_tot, int_norm, cosmology)
                N_sum += dN
                
                #Flux to Source Strength
                logmu=np.log10(flux_to_mu * erg_to_GeV*10**lum/year2sec/(4*np.pi*(Mpc_to_cm*dlz)**2)*bz) 
...
class SourceCountDistribution(object):
    def __init__(self, **kwargs):
        ...
        elif "log10mu" in kwargs.keys() and "dNdlog10mu" in kwargs.keys() and "density" in kwargs.keys():
            self.generate_from_histogram(kwargs["log10mu"], kwargs["dNdlog10mu"], kwargs["density"])
        else:
            raise NotImplementedError("You have to provide either, 'log10mu', 'dNdlog10mu', 'density' or 'file'")
    ...
    def generate_from_histogram(self, log10mu, dNdlog10mu, density):
        ...
        self.sf_spline = UnivariateSpline(np.cumsum(dNdlog10mu)[1:][mask]/np.sum(dNdlog10mu), log10mu[1:][mask], k=1, s=0)
